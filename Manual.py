...
youtubedlexe = dirPath + "/binaries/youtube-dl.exe"
...
if '1' in Content:
    ...
    if not os.path.exists(FInput_audio) and not os.path.exists(out_Audio):
        subprocess.run([youtubedlexe, '-k', '--user-agent', 'KAIOS/2.0', '-f', aud_format, '--fixup', 'never', MPD, '-o', 'aud_enc.m4a'])
    else:
        pass
else:    
    aud_format = input('\nEnter Audio Format Which You Want ? :')
    ...
    if not os.path.exists(FInput_video) and not os.path.exists(out_Video):
        subprocess.run([youtubedlexe, '-k', '--user-agent', 'KAIOS/2.0', '-f', vid_format, '--fixup', 'never', MPD, '-o', 'vid_enc.mp4'])
    else:
        pass
...
try:
    ...
    mkvmerge_command = [mkvmergeexe, '--ui-language' ,'en', '--output', output +'.mkv', '--language', '0:eng', '--default-track', '0:yes', '--compression', '0:none', Remuxed_Video, '--language', '0:eng', '--default-track', '0:yes', '--compression' ,'0:none', Demuxed_Audio,'--language', '0:tel','--track-order', '0:0,1:0,2:0,3:0,4:0']
